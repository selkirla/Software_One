...
public final class ABCDGuesser1 {
    ...
    public static void main(String[] args) {
        ...
        int countA = 0;
        while (countA < charm.length) {
            int countB = 0;

            while (countB < charm.length) {
                int countC = 0;

                while (countC < charm.length) {
                    int countD = 0;

                    while (countD < charm.length) {
                        double result = Math.pow(w, charm[countA])
                                * Math.pow(x, charm[countB])
                                * Math.pow(y, charm[countC])
                                * Math.pow(z, charm[countD]);
